...
get_pte(pde_t *pgdir, uintptr_t la, bool create) {
...
#endif
	pte_t *pd_entry = (pte_t *)(pgdir + PDX(la));
	uintptr_t pa;
	if((*pd_entry & PTE_P) == 0){
		if(create == 0) return NULL;
		struct Page* page = alloc_page();// page table
		if(page != NULL){
			set_page_ref(page, 1);
			pa = page2pa(page);
	...
	return (pte_t *)(KADDR(pa + PTX(la)*4));
}
